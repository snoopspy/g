...
// channel
GDemon::ChOpenRes GDemonClient::chOpen(std::string intfName) {
	GDemon::ChOpenRes res;
...
bool GDemonClient::chSetChannel(int channel) {
	ChSetChannel setChannel;

	setChannel.channel_ = channel;
	{
	  GSpinLockGuard guard(sendBufLock_);
	  int32_t encLen = setChannel.encode(sendBuf_, MaxBufSize);
	  if (encLen == -1) {
		error_ = "req.encode return -1";
		return false;
	  }
