...
// channel
//
int32_t GDemon::ChOpenReq::encode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;
...
int32_t GDemon::ChOpenReq::decode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;

	int32_t decLen = Header::decode(buf, size); buf += decLen; size -= decLen;
	if (cmd_ != CmdChOpen) {
	  GTRACE("cmd_ is not CmdChOpen %d", cmd_);
	  return -1;
	}
...
int32_t GDemon::ChOpenRes::encode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;
...
int32_t GDemon::ChOpenRes::decode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;
...
int32_t GDemon::ChCloseReq::encode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;
...
int32_t GDemon::ChCloseReq::decode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;

	int32_t decLen = Header::decode(buf, size); buf += decLen; size -= decLen;
	if (cmd_ != CmdChClose) {
	  GTRACE("cmd_ is not CmdChClose %d", cmd_);
	  return -1;
	}
...
int32_t GDemon::ChSetChannel::encode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;

	buf += sizeof(Header); size -= sizeof(Header);

	// channel_
	*pint32_t(buf) = channel_; buf += sizeof(channel_); size -= sizeof(channel_);

	len_ = buf - buffer - sizeof(Header);
	cmd_ = CmdChSetChannel;
	Header::encode(buffer, sizeof(Header));
...
int32_t GDemon::ChSetChannel::decode(pchar buffer, int32_t size) {
	volatile pchar buf = buffer;

	int32_t decLen = Header::decode(buf, size); buf += decLen; size -= decLen;
	if (cmd_ != CmdChSetChannel) {
	  GTRACE("cmd_ is not CmdChSetChannel %d", cmd_);
	  return -1;
	}

	// channel_
	channel_ = *pint32_t(buf); buf += sizeof(channel_); size -= sizeof(channel_);
